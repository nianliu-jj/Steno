    ...
    >
      ...
      <div class="clip-editor__toolbar" data-testid="clip-editor-toolbar">
        <div class="clip-editor__group">
          ...
          <button class="clip-editor__icon" type="button" title="缩放尺寸" data-testid="clip-tool-resize" :class="{ 'clip-editor__icon--active': showResize }" @click="openResize">
      ...
      <div ref="stageEl" class="clip-editor__stage" data-testid="clip-editor-stage">
        ...
        <div v-if="cropping" class="clip-editor__crop-layer" data-testid="clip-crop">
          ...
          >
            <span class="clip-editor__crop-handle clip-editor__crop-handle--tl" data-testid="clip-crop-handle-tl" @pointerdown="startCropResize($event, 'tl')" />
            <span class="clip-editor__crop-handle clip-editor__crop-handle--tr" data-testid="clip-crop-handle-tr" @pointerdown="startCropResize($event, 'tr')" />
            <span class="clip-editor__crop-handle clip-editor__crop-handle--bl" data-testid="clip-crop-handle-bl" @pointerdown="startCropResize($event, 'bl')" />
            <span class="clip-editor__crop-handle clip-editor__crop-handle--br" data-testid="clip-crop-handle-br" @pointerdown="startCropResize($event, 'br')" />
          </div>
        ...
        <div v-if="showResize" class="clip-editor__resize-panel" data-testid="clip-resize">
          <label>宽度
            <input v-model.number="resizeW" type="number" min="1" data-testid="clip-resize-width" @input="onResizeWidthInput">
          </label>
          <label>高度
            <input v-model.number="resizeH" type="number" min="1" data-testid="clip-resize-height" @input="onResizeHeightInput">
          </label>
          <label class="clip-editor__resize-lock"><input v-model="lockRatio" type="checkbox" data-testid="clip-resize-lock"> 锁定纵横比</label>
          <div class="clip-editor__resize-actions">
            <button class="clip-editor__btn" type="button" data-testid="clip-resize-confirm" @click="confirmResize">应用</button>
            <button class="clip-editor__btn" type="button" data-testid="clip-resize-cancel" @click="showResize = false">取消</button>
          </div>
        </div>

        <div v-if="showAdjust" class="clip-editor__adjust" data-testid="clip-adjust">
          <label>亮度
            <input v-model.number="draft.brightness" type="range" min="-100" max="100" data-testid="clip-adjust-brightness" @change="commitAdjust">
          </label>
          <label>对比度
            <input v-model.number="draft.contrast" type="range" min="-100" max="100" data-testid="clip-adjust-contrast" @change="commitAdjust">
          </label>
          <label>饱和度
            <input v-model.number="draft.saturation" type="range" min="-100" max="100" data-testid="clip-adjust-saturation" @change="commitAdjust">
          </label>
          <div class="clip-editor__adjust-toggles">
            <label><input v-model="draft.grayscale" type="checkbox" data-testid="clip-adjust-grayscale" @change="commitAdjust"> 灰度</label>
            <label><input v-model="draft.invert" type="checkbox" data-testid="clip-adjust-invert" @change="commitAdjust"> 反相</label>
          </div>
        </div>
      </div>
      ...
      <footer class="clip-editor__footer" data-testid="clip-editor-footer">
        ...
        <span class="clip-editor__save-split">
          <button class="clip-editor__primary" type="button" :disabled="saving || !editor.dirty.value" data-testid="clip-editor-save" @click="saveAsNew">保存为新条目</button>
          <button class="clip-editor__primary-caret" type="button" title="更多保存方式（覆盖原图 / 导出文件，即将支持）" data-testid="clip-editor-save-more" disabled>▾</button>
        </span>
      </footer>
...
.clip-editor__crop-layer { position: absolute; inset: 18px; pointer-events: none; }
...
.clip-editor__crop-handle--tl { left: -6px; top: -6px; cursor: nwse-resize; }
.clip-editor__crop-handle--tr { right: -6px; top: -6px; cursor: nesw-resize; }
.clip-editor__crop-handle--bl { left: -6px; bottom: -6px; cursor: nesw-resize; }
.clip-editor__crop-handle--br { right: -6px; bottom: -6px; cursor: nwse-resize; }
...
.clip-editor__resize-panel label { display: grid; gap: 4px; font-size: 12px; color: var(--app-muted); }
...
.clip-editor__resize-lock { flex-direction: row; align-items: center; }
.clip-editor__resize-lock { display: inline-flex; gap: 6px; }
.clip-editor__resize-actions { display: flex; gap: 6px; }
.clip-editor__adjust {
  position: absolute;
  top: 10px;
  right: 10px;
...
.clip-editor__icon:hover { border-color: var(--app-accent); color: var(--app-accent); }
